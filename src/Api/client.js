...
export function generateJwtApiClient({
  baseUrl = "",
  timeout = 5000,
  authorizationHeader = "Authorization",
  authorizationPrefix = "Bearer ",
  ...
  apiClient.interceptors.request.use(
    async function (config) {
      ...
      let refreshToken =
        ...
        sessionStorage.getItem(localStorageKey);
      let accessTokenStorage = localStorage.getItem(accessTokenLocalStorageKey)
        ? localStorage
        : sessionStorage;
      let refreshTokenStorage = localStorage.getItem(
      ...
      if (accessToken) {
        ...
        if (
          ...
          try {
            ...
            accessTokenStorage.setItem(
              accessTokenLocalStorageKey,
              newAccessToken
            );
            refreshTokenStorage.setItem(
              refreshTokenLocalStorageKey,
              newRefreshToken
            );
